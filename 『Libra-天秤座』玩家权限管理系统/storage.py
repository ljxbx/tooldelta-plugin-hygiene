...
class StateStore:
    ...
    def _convert_keys(cls, value: Any) -> Any:
        if isinstance(value, dict):
            return {cls._KEYS.get(str(k), str(k)): cls._convert_keys(v) for k, v in value.items()}
        ...
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                ...
                if chinese_key == "期望权限" and isinstance(item, dict):
                    item = [{"玩家标识": str(xuid), "权限标志": flags} for xuid, flags in item.items()]
    ...
    def _decode(cls, value: Any) -> Any:
        if isinstance(value, dict):
            decoded = {cls._KEYS.get(str(k), str(k)): cls._decode(v) for k, v in value.items()}
            if "期望权限" in decoded and isinstance(decoded["期望权限"], list):
                decoded["期望权限"] = {
                    str(item.get("玩家标识")): item.get("权限标志")
                    for item in decoded["期望权限"]
                    if isinstance(item, dict) and item.get("玩家标识") and item.get("权限标志") is not None
                }
            return decoded
        if isinstance(value, list):
    ...
    def load(self) -> dict[str, Any]:
        ...
        if not isinstance(state["期望权限"], dict): state["期望权限"] = {}
        if not isinstance(state["已发现管理员"], list): state["已发现管理员"] = []
        if not isinstance(state["操作记录"], list): state["操作记录"] = []
        if not isinstance(state["权限快照"], list): state["权限快照"] = []
        if not isinstance(state["最近成功管理员"], list): state["最近成功管理员"] = []
        if not isinstance(state["最近搜索结果"], list): state["最近搜索结果"] = []
        return state
    ...
    def _migrate_legacy_files(self) -> None:
            ...
            try:
                ...
                if isinstance(old, dict):
                    ...
                    self.state_path.write_text(
                        json.dumps(self._encode(old), ensure_ascii=False, indent=2), encoding="utf-8"
                    )
        ...
        if not self.audit_path.exists() and legacy_audit.exists():
            try:
                with legacy_audit.open("r", encoding="utf-8") as source, self.audit_path.open("w", encoding="utf-8") as target:
                    for line in source:
                        try:
                            record = json.loads(line)
                            target.write(json.dumps(self._encode(record), ensure_ascii=False) + "\n")
